...
def boggle_helper(s_n, pos, used_pos, ans_lst, ch_map, dic):
	...
	# Prune
	if len(s_n) == 4 and s_n not in dic:			# if no prefix is string_now, return this chosen
		used_pos.pop()								# return used_pos too
		return

	# when found word, print it then keep search
	if len(s_n) >= 4 and (s_n in dic[s_n[:4]]) and (s_n not in ans_lst):
